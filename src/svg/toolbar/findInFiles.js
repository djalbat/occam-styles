"use strict";

import ToolbarSVG from "../../svg/toolbar";

export default class FindInFilesToolbarSVG extends ToolbarSVG {
  childElements() {
    return (

      <g>
        <rect width="3.5" height="1.5" x="15" y="9.5" />
        <rect width="1.5" height="3.75" x="17" y="5.75" />
        <rect width="1.5" height="2.5" x="11" y="3.5" />
        <rect width="3.75" height="1.5" x="11" y="2" />
        <path d="M 14.75,2 18.5,5.75 H 14.75 Z" />
        <rect width="2.5" height="1.5" x="18.5" y="7.5" />
        <rect width="1.5" height="3.75" x="19.5" y="3.75" />
        <rect width="1.5" height="1" x="13.5" y="1.5" />
        <rect width="3.75" height="1.5" x="13.5" y="0" />
        <path d="M 17.25,0 21,3.75 h -3.75 z" />
        <rect width="2.5" height="3" x="9.5" y="7" />
        <rect width="5" height="2.5" x="9.5" y="10" />
        <path d="m 9.8253217,5.0027954 a 6,6 0 0 0 -4.067968,1.7544149 6,6 0 0 0 0,8.4852697 6,6 0 0 0 7.7163243,0.645439 l 0.768945,0.768945 -0.353466,0.353467 5.656502,5.656502 a 1.55,1.55 0 0 0 2.121317,5.17e-4 1.55,1.55 0 0 0 0,-2.121834 l -5.656502,-5.656502 -0.353467,0.353983 -0.766878,-0.766878 A 6,6 0 0 0 14.242623,6.7572103 6,6 0 0 0 9.8253217,5.0027954 Z m -0.03669,2.5037232 a 3.4999826,3.4999824 0 0 1 2.6866573,1.0185424 3.4999826,3.4999824 0 0 1 -5.16e-4,4.950085 3.4999826,3.4999824 0 0 1 -4.9495693,0 3.4999826,3.4999824 0 0 1 0,-4.950085 3.4999826,3.4999824 0 0 1 2.263428,-1.0185424 z" />
      </g>

    );
  }

  static defaultProperties = {
    className: "find-in-files"
  };
}
